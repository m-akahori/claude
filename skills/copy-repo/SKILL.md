---
name: copy-repo
description: Gitリポジトリをコピーして新しいプロジェクトを作成する
argument-hint: "<元のリポジトリ名> <新しいリポジトリ名>"
disable-model-invocation: true
allowed-tools: Bash(cp *) Bash(rm *) Bash(cd *) Bash(git *) Bash(composer *) Bash(npm *) Bash(php *)
---

Gitリポジトリをコピーして新しいプロジェクトを作成します。

使用例: `/copy-repo <元のリポジトリ名> <新しいリポジトリ名>`

例: `/copy-repo regrit idol`
...
if [ -z "{{arg1}}" ] || [ -z "{{arg2}}" ]; then
  echo "エラー: 引数が不足しています。"
  echo "使用例: /copy-repo <元のリポジトリ名> <新しいリポジトリ名>"
  exit 1
fi
...
.envファイルのAPP_NAMEを新しいリポジトリ名に変更します。
...
異なるポート番号を使用する場合は、.envファイルを編集してください。
...
```bash
echo ""
echo "新しいGitリポジトリとして初期化しますか？ (y/n)"
read -r INIT_GIT
...
echo "2. 開発環境を起動: /start-dev ${TARGET_REPO}"
